...
const WeatherWidget: React.FC<Weather> = ({ date, weather, temp, isFahrenheit }) => {
    ...
    return (
        <div className="flex flex-col font-raleway bg-[#1E213A] w-[120px] h-[177px] items-center justify-between  py-3">
            ...
            <div className="flex w-[70%] justify-between">
                ...
                <p className="text-[#E7E7EB] text-[16px]">{`${isFahrenheit ? `${Math.floor(temp[0] * (9 / 5) + 32.5)}ºF` : `${Math.floor(temp[0] + 0.5)}ºC`}`} </p>
                <p className="text-[#A09FB1] text-[16px]">{`${isFahrenheit ? `${Math.floor(temp[1] * (9 / 5) + 32.5)}ºF` : `${Math.floor(temp[1] + 0.5)}ºC`}`} </p>
            </div>
